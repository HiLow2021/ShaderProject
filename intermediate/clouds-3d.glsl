...
vec2 map(vec3 p)
{
    vec3 p2=p;
    p2.xy-=disp(p.z).xy;
    p.xy*=rot(sin(p.z+u_time)*(.1+prm1*.05)+u_time*.09);
    float cl=mag2(p2.xy);
    float d=0.;
    p*=.61;
    float z=1.;
    ...
    for(int i=0;i<5;i++)
    {
        p+=sin(p.zxy*.75*trk+u_time*trk*.8)*dspAmp;
...
void main(void)
{
    ...
    bsMo=(.5*u_resolution.xy)/u_resolution.y;
    
    float time=u_time*3.;
    vec3 ro=vec3(0,0,time);
    
    ro+=vec3(sin(u_time)*.5,sin(u_time*1.)*0.,0);
    ...
    prm1=smoothstep(-.4,.4,sin(u_time*.3));
