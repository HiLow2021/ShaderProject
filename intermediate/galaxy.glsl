...
void main(void){
    ...
    float freqs[4];
    //Sound
    freqs[0]=noise(vec3(.01*100.,.25,u_time/10.));
    freqs[1]=noise(vec3(.07*100.,.25,u_time/10.));
    freqs[2]=noise(vec3(.15*100.,.25,u_time/10.));
    freqs[3]=noise(vec3(.30*100.,.25,u_time/10.));
    ...
    gl_FragColor=mix(freqs[3]-.3,1.,v)*vec4(1.5*freqs[2]*t*t*t,1.2*freqs[1]*t*t,freqs[3]*t,1.)+c2+starcolor;
}